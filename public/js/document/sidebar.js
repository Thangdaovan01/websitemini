...
$(document).ready(function() {
    ...
    $(document).on('click', '.my-document-like', function(event) {
        event.stopPropagation();

        var userId = $(this).closest('.sidebar').data('user-id');
        const likePostIds = likesArr
                            .filter(like => like.userId === userId)
                            .map(like => like.likePostId);

        const postDocumentIds = postsArr
            .filter(post => likePostIds.includes(post._id))
            .map(post => post.documentId);

        const userDocuments = documentsArr.filter(document => postDocumentIds.includes(document._id));

        const user = usersArr.find(user => user._id === userId);
        const fullname = user ? user.fullname : null;

        showDocuments(userDocuments,`Tài liệu yêu thích của bạn`);
    })
    ...
    $(document).on('click', '.document-school', function(event) {
        event.stopPropagation();
        $("#schoolSelect").toggle();
        
    })

    $(document).on('change', '#schoolSelect', function(event) {
        const selectedValue = $(this).val();
        history.pushState(null, '', `/${selectedValue}`);
        const schoolArray = documentsArr.filter(object => object.school == selectedValue);
        showDocuments(schoolArray,`Tài liệu trường ${selectedValue}`);
    });
